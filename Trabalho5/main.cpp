    ...
    while (!WindowShouldClose())
    {
        ...
        BeginDrawing();
            ...
            if (spawnPos.x >= 0 && spawnPos.y >= 0) {
                DrawRectangle(spawnPos.x * cellSize, spawnPos.y * cellSize, cellSize, cellSize, RED);
                DrawText("GERAÇÃO", spawnPos.x * cellSize, spawnPos.y * cellSize - 15, 10, RED);
            }
            if (targetPos.x >= 0 && targetPos.y >= 0) {
                DrawRectangle(targetPos.x * cellSize, targetPos.y * cellSize, cellSize, cellSize, RED);
                DrawText("ALVO", targetPos.x * cellSize, targetPos.y * cellSize - 15, 10, RED);
            }

            DrawText("Clique esq: Colocar obstáculo", 10, 10, 20, DARKGRAY);
            DrawText("Clique dir: Remover obstáculo / Colocar geração/alvo", 10, 35, 20, DARKGRAY);
            DrawText("S: Modo geração | T: Modo alvo", 10, 60, 20, DARKGRAY);
            DrawText("ENTER: Criar agente | R: 5 agentes aleatórios", 10, 85, 20, DARKGRAY);
            DrawText("P: Testes de performance | M: Salvar métricas", 10, 110, 20, DARKGRAY);
            DrawText("ESC: Cancelar modo de posicionamento", 10, 135, 20, DARKGRAY);
            DrawText(TextFormat("Agentes: %d", agentManager.GetAgentCount()), 10, 160, 20, DARKGRAY);
            
            if (placingSpawn) {
                DrawText("MODO: Posicionando GERAÇÃO (Clique dir para colocar)", 10, 185, 20, RED);
            } else if (placingTarget) {
                DrawText("MODO: Posicionando ALVO (Clique dir para colocar)", 10, 185, 20, RED);
            }
